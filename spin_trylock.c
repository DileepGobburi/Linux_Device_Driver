...
int my_thread1(void *p){
	...
	if(spin_trylock(&my_spinlock))
	  ...
	  printk(KERN_INFO"spinlock(my_thread1) locked status : %d\n",spin_is_locked(&my_spinlock));
	...
	msleep(1000);
         ...
         {
           pr_info("lock is not acquired by thread1");
         }
         }
	return 0;
}
...
int my_thread2(void *p){
	...
	printk(KERN_INFO"spinlock(my_thread2) locked status : %d\n",spin_is_locked(&my_spinlock));
