...
MODULE_DESCRIPTION("Kernel Signal");
...
{
	//struct siginfo info;         --> for pervious kernel versions
    struct kernel_siginfo info;    // lastest kernel versions
...
static int __init my_driver_init(void)
{
	...
	if((device_create(dev_class,NULL,dev,NULL,"my_signal_driver"))==NULL) {
		printk(KERN_INFO" cannot create the device\n");
		goto r_device;
	}
	
	if (request_irq(IRQ_NO, irq_handler, IRQF_SHARED, "my_signal_driver", (void *)(irq_handler))) {
